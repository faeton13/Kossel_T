...
module frame_cheek(double_frame,estp_sw)
 for (i=[-1,1])
  difference ()  {
    union () {
      frame_axis_block(h_up_frame, w_up_frame, thickness, extr, diagonal,extr*0.9, extr/2,m_frame_open*2);
...
///flexible cutt off  
  ...
  translate([-2/2,0,-h_up_frame])
   rotate(270,0,0)
    cube([diagonal*2,slot_hole,h_up_frame*2]);
  
    translate ([-diagonal/2*i,0,-h_up_frame])
        cylinder (r=3,h=h_up_frame*2, $fn=16);
    translate ([-diagonal/4*i,0,0])
        cube ([diagonal/2,3*2,h_up_frame+4], center=true);
   }
